...
function createComment(url, request) {
  ...
  if (requestComment && requestComment.body && database.articles[requestComment.articleId] && requestComment.username && database.users[requestComment.username]) {
    const comment = {
...
function deleteComment(url, request) {
  ...
  if (savedComment) {
    ...
    response.userListId = database.users[savedComment.username].commentIds.splice(userIndex, 1);

    const articleIndex = database.articles[savedComment.articleId].commentIds.indexOf(id);
    response.articleListId = database.articles[savedComment.articleId].commentIds.splice(articleIndex, 1);
